...
load('roughness_parameters_not_symmetric.mat'); % the computed coefficients W1 to W14
...
for k=1:size(combinations,1)
    ...
    [p,tbl,stats] = kruskalwallis(reshape(y_verify(:,1),[no_images_per_texture,no_images/no_images_per_texture]),[],'off');
    ...
    combinations{k,6} = sum(pValues(find(pValues < correctedAlpha))) / numel(pValues);
...
save('roughness_best_parameter_configuration_not_symmetric_new_accuracy_I.mat','combinations');