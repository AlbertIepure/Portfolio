...
for i=1208:size(combinations,1)
    i
    feature_config = combinations{i,1};
...
%     text(73,71,message);
    ...
    saveTightFigure(h,'ConfusionMatrix5F_magnified.pdf');
%     combinations{i,2} = (1-error_rate_CV)*100;
end

for i=1:size(combinations,1)
    combinations{i,2} = recognition_rate_1(i);
    combinations{i,3} = recognition_rate_2(i);
end
toc

elem = [];
 for i=1:2047 
     combinations{i,4} = size(combinations{i,1},2);
     elem(i) = combinations{i,4} ; 
 end
 b =[];
 for i=1:11
     b(i)=min(find(elem ==i))
...
save('all_feature_combinations_success_rate_kernel_epanechnikov.mat','combinations')