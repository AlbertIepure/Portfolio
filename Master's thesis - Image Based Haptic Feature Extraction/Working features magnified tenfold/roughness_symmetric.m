function Froughness = roughness_symmetric(I)
% Good theory:
...
% P_H  = graycomatrix(A, 'NumLevels',4, 'offset', [0 1], 'Symmetric', true);
% P_V  = graycomatrix(A, 'NumLevels',4, 'offset', [-1 0], 'Symmetric', true);
% P_LD = graycomatrix(A, 'NumLevels',4, 'offset', [-1 -1], 'Symmetric', true);
% P_RD = graycomatrix(A, 'NumLevels',4, 'offset', [-1 1], 'Symmetric', true);

% I = [0 0 1 1; 0 0 1 1; 0 2 2 2; 2 2 3 3];
[Ny,Nx] = size(I);
Ng = 32;
% GLCM computation
P_H  = graycomatrix(I, 'NumLevels',Ng, 'GrayLimits', [], 'offset', [0 1], 'Symmetric', true); % 0°
R = 2*Ny*(Nx-1); % neighbroing resolution cell pairs
p_H = P_H./R; % normalization

P_V  = graycomatrix(I, 'NumLevels',Ng, 'GrayLimits', [], 'offset', [-1 0], 'Symmetric', true); % 90 °
R = 2*Nx*(Ny-1); % neighbroing resolution cell pairs
p_V = P_V./R; % normalization

P_LD = graycomatrix(I, 'NumLevels',Ng, 'GrayLimits', [], 'offset', [-1 -1], 'Symmetric', true); % 135°
R = 2*(Nx-1)*(Ny-1); % neighbroing resolution cell pairs
p_LD = P_LD./R; % normalization

P_RD = graycomatrix(I, 'NumLevels',Ng, 'GrayLimits', [], 'offset', [-1 1], 'Symmetric', true); % 45°
