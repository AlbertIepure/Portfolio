...
for k=1:N:height
    for j=1:N:width
        ...
        for i=1:dimensions
            ...
            zero_run_code = [zero_run_code , aZeroRunEnc(row_QUANTIZED_block(2:end))]; %the AC coefficients are zero-run encoded
        end
    end
end
...
for i=1:3*width/N:length(DC_coefficients)
    DC_coefficients_2 = [DC_coefficients(i:i + 3*width/N - 1) - [0 0 0,DC_coefficients(i:i + 3*width/N - 1 -3)]];
    DC_unencoded = [DC_unencoded, DC_coefficients_2(1:3)];
    DC_differential = [DC_differential, DC_coefficients_2(4:end)];
end
...
nr_symbols_to_encode_DC = max(size(DC_differential));
...
HUFFMAN_encoded_bitstream_DC = [DC_unencoded.';enc_huffman_new( DC_differential + abs(lower_bound_DC) + 1, BinCode_DC, Codelengths_DC)];
