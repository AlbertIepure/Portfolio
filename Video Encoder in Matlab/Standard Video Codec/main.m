...
for akSet = 1:SetSize                         % and loop through all of them
    ...
    MSE(akSet) = calcMSE(RECONSTRUCTED_image, ORIGINAL_image, ORIGINAL_height, ORIGINAL_width, ORIGINAL_dimensions);
    PSNR(akSet) = calcPSNR(MSE(akSet), RECONSTRUCTED_image);
    ...
    bitrate(akSet) = COMPRESSED_size(1)*8/(ORIGINAL_height * ORIGINAL_width);
...
save('F:/IVC_labs_starting_point/exercises/starting_algorithm','bitrate','PSNR'); 
