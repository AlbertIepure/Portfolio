...
for f = 2:21
    ...
    reference_frame = DECODED_YCbCr_image;
     ...
     %% MVFAST algorithm
    tic
    [mv_mat,contor(f-1)] = MVFAST(current_frame, reference_frame, block_size, search_range, height, width);
    [mv_mat,c] = half_pel_ME(current_frame, reference_frame, mv_mat, block_size, search_range, height, width, half_pel);
    contor(f-1) = contor(f-1) + c;
    t(f-1) = toc;
    ...
    mv_row = mv_row * power(2, half_pel);
    
    if f==2
        H_2 = hist(mv_row,-power(2, half_pel)*search_range:power(2, half_pel)*search_range);
        H_2 = H_2/sum(H_2);
    ...
    HUFFMAN_encoded_bitstream_2 = enc_huffman_new( mv_row + power(2, half_pel)*search_range + 1 , BinCode_2, Codelengths_2);
    ...
    for i=1:block_size:height
        for j=1:block_size:width
            ...
            elseif mod(y,1) == 0
                x_1 = ceil(x);
                x_2 = floor(x);
                current_predicted_frame(i:i+block_size-1,j:j+block_size-1,:) = (reference_frame(i+y:i+y+block_size-1,j+x_1:j+x_1+block_size-1,:) + reference_frame(i+y:i+y+block_size-1,j+x_2:j+x_2+block_size-1,:) + ones(block_size,block_size,3))./2;
            elseif mod(x,1) == 0
                y_1 = ceil(y);
                y_2 = floor(y);
                current_predicted_frame(i:i+block_size-1,j:j+block_size-1,:) = (reference_frame(i+y_1:i+y_1+block_size-1,j+x:j+x+block_size-1,:) + reference_frame(i+y_2:i+y_2+block_size-1,j+x:j+x+block_size-1,:) + ones(block_size,block_size,3))./2;
            else
                x_1 = ceil(x);
                x_2 = floor(x);
                y_1 = ceil(y);
                y_2 = floor(y);
                current_predicted_frame(i:i+block_size-1,j:j+block_size-1,:) = (reference_frame(i+y_1:i+y_1+block_size-1,j+x_1:j+x_1+block_size-1,:) + reference_frame(i+y_2:i+y_2+block_size-1,j+x_1:j+x_1+block_size-1,:) + reference_frame(i+y_1:i+y_1+block_size-1,j+x_2:j+x_2+block_size-1,:) + reference_frame(i+y_2:i+y_2+block_size-1,j+x_2:j+x_2+block_size-1,:) + 2*ones(block_size,block_size,3))./4;
            end 
            block_nb = block_nb + 1;
        end
    end
    ...
    [HUFFMAN_encoded_bitstream_3, nr_symbols_to_encode_3] = IntraEncode(ERROR_frame, N, M, BinCode_3, Codelengths_3, lower_bound_3);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Decode the ERROR_frame and the motion vector
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    DECODED_ERROR_image = IntraDecode (HUFFMAN_encoded_bitstream_3, BinaryTree_3, nr_symbols_to_encode_3, height, width, dimensions, N, M, lower_bound_3);
    ...
    DECODED_mv =  double(vec2mat(dec_huffman ( HUFFMAN_encoded_bitstream_2, BinaryTree_2, max(size(mv_row(:)))), size(mv_mat,2))) - power(2, half_pel)*search_range - 1;
    DECODED_mv = DECODED_mv./power(2, half_pel);
    ...
    Bitrate(f) = (length(HUFFMAN_encoded_bitstream_2)+length(HUFFMAN_encoded_bitstream_3))*8 / width / height;
end
