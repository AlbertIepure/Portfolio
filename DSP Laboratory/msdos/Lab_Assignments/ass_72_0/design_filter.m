function design_filter(F_c,F_stop,F_s,a_s)
% F_c: cut-off frequency
% F_stop: stop-band edge frequency
% F_s: sampling frequency
% a_s: attenuation at frequency F_stop in dB

F_c = F_s/2 - F_c;%design the LPF according to the specifications of the HPF
F_stop = F_s/2 - F_stop;
...
for k = 0:N-1
    ...
    H_z(k+1,:) = (z-1)./(z+z_k(k+1));%transfer function of the HPF
    H_total = H_total .* H_z(k+1,:);
end


H_total = H_total/max(H_total);
h = ifft(H_total);
...
zero = ones(1,N); %the zeros of the HPf are 1
pole = -z_k;      %the poles of the HPF
