...
for n = 0:2*M
    if n == M
        h(n+1) = 1-(omega_c2-omega_c1)/pi;
    else
        h(n+1) = (omega_c1/pi)*(sin(omega_c1*(n-M))/(omega_c1*(n-M))) - (omega_c2/pi)*(sin(omega_c2*(n-M))/(omega_c2*(n-M)));
    end;
end;
...
%     saveas(gcf,'magnitude_and_frequency_plot.jpg');
    fvtool(h,1,y_mult,1);
    legend('BSF with rectangular window','BSF with Hamming window');
end