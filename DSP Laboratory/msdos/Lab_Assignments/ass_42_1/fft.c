...
void fft(float array_to_transform_re[],float array_to_transform_im[],int length_array) 
{
    ...
    for(i=0;i<length_array;i++) //Copy of the input signal in a complexe array
     {
        arrayfft[i].re =array_to_transform_re[i];
        arrayfft[i].im =0;
        scrambledarray[i]=i;
    }
	...
	for(i=0;i<length_array;i++) //Unscrambling and printing process
    {
            ...
            array_to_transform_re[i] = arrayfft[unscrambled].re ; 
            array_to_transform_im[i] = arrayfft[unscrambled].im ;
    }
